...
/* ------------------------------------------------------------------ */
/* The SMPC persistent block, on disk                                  */
/* ------------------------------------------------------------------ */
/*
 * Ymir 0.4.0 stopped reading and writing this file itself.
 *
 * Up to 0.3.x, SMPC::LoadPersistentDataFrom/SavePersistentDataTo took a path
 * and owned the format, so this bridge round-tripped through a temp file just
 * to get at the bytes. 0.4.0 hands over a plain PersistentSMPCData struct
 * instead and leaves the file to the frontend -- which is better in every way
 * except that the format is now ours to keep.
 *
 * So it is kept, byte for byte, exactly as 0.3.x wrote it:
 *
 *     [0]      version, 0x01
 *     [1..3]   reserved, zero
 *     [4..7]   SMEM
 *     [8]      STE
 *     [9..16]  RTC offset,    int64, host order
 *     [17..24] RTC timestamp, int64, host order
 *
 * Twenty-five bytes, which is what YMIR_SMPC_STATE_SIZE has always said. A
 * different layout here would silently reset every existing user's Saturn
 * clock and language -- small things, but ones they chose.
 */
static constexpr uint8_t kSmpcPersistVersion = 0x01;

static void smpc_state_encode(const ymir::smpc::PersistentSMPCData &pd,
                              std::vector<uint8_t> &out) {
    out.assign(YMIR_SMPC_STATE_SIZE, 0);
    out[0] = kSmpcPersistVersion;                       /* [1..3] stay zero */
    std::memcpy(&out[4], pd.SMEM.data(), pd.SMEM.size());
    out[8] = pd.STE ? 1 : 0;
    const int64_t offset = pd.rtc.offset;
    const int64_t timestamp = pd.rtc.timestamp;
    std::memcpy(&out[9], &offset, sizeof(offset));
    std::memcpy(&out[17], &timestamp, sizeof(timestamp));
}

static bool smpc_state_decode(const std::vector<uint8_t> &in,
                              ymir::smpc::PersistentSMPCData &pd) {
    if (in.size() != YMIR_SMPC_STATE_SIZE) return false;
    if (in[0] != kSmpcPersistVersion) return false;
    std::memcpy(pd.SMEM.data(), &in[4], pd.SMEM.size());
    pd.STE = in[8] != 0;
    int64_t offset = 0, timestamp = 0;
    std::memcpy(&offset, &in[9], sizeof(offset));
    std::memcpy(&timestamp, &in[17], sizeof(timestamp));
    pd.rtc.offset = offset;
    pd.rtc.timestamp = timestamp;
    return true;
}
...
static int32_t apply_core_option(YmirInstance *inst, YmirCoreOption option,
    ...
    case YMIR_OPT_SH2_OVERCLOCK:
        ...
        /* A Ratio, not a percentage, since 0.4.0. The option's unit on the
         * C ABI stays a percentage -- that is what every caller already
         * passes and what the UI shows -- so it is converted here rather
         * than changing the ABI under the frontends. */
        cfg.system.sh2ClockFactor = RatioU32::FromPercentage((uint32_t)value);
        return YMIR_OK;
    case YMIR_OPT_THREADED_VDP1:
        cfg.swRenderer.threadedVDP1 = flag;
        return YMIR_OK;
    case YMIR_OPT_THREADED_VDP2:
        cfg.swRenderer.threadedVDP2 = flag;
        return YMIR_OK;
    case YMIR_OPT_THREADED_DEINTERLACE:
        cfg.swRenderer.threadedDeinterlacer = flag;
        return YMIR_OK;
...
static void apply_request(YmirInstance *inst, Request *req) {
    ...
    try {
        ...
        case Request::kLoadSmpc: {
            ...
            if (rc != YMIR_OK) break;
            ymir::smpc::PersistentSMPCData pd{};
            if (!smpc_state_decode(data, pd)) { rc = YMIR_ERR_BAD_FORMAT; break; }
...
int32_t ymir_bridge_get_core_option(YmirInstance *inst, YmirCoreOption option) {
    ...
    case YMIR_OPT_SH2_OVERCLOCK: {
        /* Back to a percentage, the unit the ABI promises. */
        const auto r = *cfg.system.sh2ClockFactor;
        return (int32_t)((r.Numerator() * 100u) / r.Denominator());
    }
    case YMIR_OPT_THREADED_VDP1:        return *cfg.swRenderer.threadedVDP1 ? 1 : 0;
    case YMIR_OPT_THREADED_VDP2:        return *cfg.swRenderer.threadedVDP2 ? 1 : 0;
    case YMIR_OPT_THREADED_DEINTERLACE: return *cfg.swRenderer.threadedDeinterlacer ? 1 : 0;
    case YMIR_OPT_AUDIO_INTERPOLATION:
...
void ymir_bridge_set_persistent_smpc_path(YmirInstance *inst, const char *path) {
    if (!inst || !path) return;
    /*
     * Load the SMPC persistent block, if there is one.
     *
     * Up to 0.3.x this handed ymir-core a path and the core read the file
     * during boot. 0.4.0 does not touch the filesystem at all, so the read
     * and the decode are ours -- see smpc_state_decode above, which keeps
     * 0.3.x's exact byte layout so an existing file still loads.
     *
     * A missing or unreadable file is fine and silent: it is what a first
     * run looks like, and the Saturn simply starts with its defaults.
     */
    int32_t rc = YMIR_OK;
    auto data = read_file(path, &rc);
    if (rc != YMIR_OK) return;
    ymir::smpc::PersistentSMPCData pd{};
    if (smpc_state_decode(data, pd)) {
        inst->saturn->SMPC.LoadPersistentData(pd);
    }
}
